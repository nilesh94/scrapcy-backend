...
class RoleChecker:
    ...
    async def __call__(
        ...
        # ==== COMMENTED FOR TESTING - UNCOMMENT WHEN AUTH READY ====
        # user_role = current_user.get("role") or current_user.role
        # 
...
RequireSeller = Depends(RoleChecker(["SELLER", "ADMIN"]))

# For buyer-only endpoints
RequireBuyer = Depends(RoleChecker(["BUYER", "ADMIN"]))

# For admin-only endpoints
RequireAdmin = Depends(RoleChecker(["ADMIN"]))

# For L1 approver
RequireL1Approver = Depends(RoleChecker(["L1_APPROVER", "ADMIN"]))

# For L2 approver
RequireL2Approver = Depends(RoleChecker(["L2_APPROVER", "ADMIN"]))
...
async def get_my_bids(
    current_user: dict = RequireAuth  # Just need to be logged in
):
    pass
...
async def create_auction(
    current_user: dict = RequireSeller  # Only SELLER or ADMIN
):
    pass
...
async def approve_auction(
    current_user: dict = RequireAdmin  # Only ADMIN
):
    pass
...
@router.post("/bid")
async def place_bid(
    current_user: dict = RequireBuyer  # Only BUYER or ADMIN
):
    pass
