...
def create_requirement(
    ...
    try:
        ...
        if current_user:
            ...
            # Validate Guest Fields (snake_case access)
            if not all([req.guest_name, req.guest_email, req.guest_phone, req.guest_company, req.guest_gst]):
                 raise HTTPException(
                     status_code=400, 
                     detail="All guest details (Name, Email, Phone, Company, GST) are mandatory for non-logged in users."
                 )
            
            db_req.guest_name = req.guest_name
            db_req.guest_email = req.guest_email
            db_req.guest_phone = req.guest_phone
            db_req.guest_company = req.guest_company
            db_req.guest_gst = req.guest_gst
